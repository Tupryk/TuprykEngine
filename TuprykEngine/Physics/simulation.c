...
float effective_mass_from_root_joint(config* C, int root_id, tensor* point, tensor* normal)
{
    ...
    {
        tensor_sub(point, jf->pos, r_a);
    }
    else if (jd->type == 3)
    {
        tensor_sub(point, jd->com, r_a);
    }
    tensor* r_a_cross_n = vector_cross_give(r_a, normal);
    tensor* I_inv_r_a = tensor_mult_give(jd->I_cm_inv, r_a_cross_n);
    tensor* r_a_term = vector_cross_give(I_inv_r_a, r_a);
    
    float effective_mass = (1.f / jd->total_mass) + vector_dot(r_a_term, normal);
...
void load_collision_forces_to_joints(config* C)
{
    ...
    while (current_contact_elem != NULL)
    {
        ...
        float normal_mag = contact->depth * correction_strength;
        tensor* contact_normal_a = tensor_scalar_mult_give(contact->normal, -normal_mag);
        tensor* contact_normal_b = tensor_scalar_mult_give(contact->normal,  normal_mag);
