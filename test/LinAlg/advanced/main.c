#define DEBUG

#include <stdio.h>
#include "../../../TuprykEngine/ui/prints/linalg.h"
#include "../../../TuprykEngine/LinAlg/tensor.h"


int test_lu_decomposition_single(tensor_t* A)
{
    printf("----- A -----\n");
    print_tensor(A);
    int operator_shape[] = {A->shape[0], A->shape[0]};
    tensor_t* P = new_tensor(operator_shape, 2, NULL);
    tensor_t* L = tensor_copy_shape(P);
    tensor_t* U = tensor_copy_shape(A);
    tensor_lu_decomp(A, P, L, U);
    tensor_t* PL = tensor_mult_give(P, L);
    tensor_t* A_reconstructed = tensor_mult_give(PL, U);

    printf("----- P -----\n");
    print_tensor(P);
    printf("----- L -----\n");
    print_tensor(L);
    printf("----- U -----\n");
    print_tensor(U);
    printf("----- A Reconstructed -----\n");
    print_tensor(A_reconstructed);

    int failure = !tensors_equal(A, A_reconstructed);

    tensor_free(P);
    tensor_free(L);
    tensor_free(U);
    tensor_free(PL);
    tensor_free(A_reconstructed);

    return failure;
}

int test_lu_decomposition()
{
    int failure_count = 0;

    printf("--- Test 1 a): LU Decomposition ---\n");
    
    int a_shape_0[] = {3, 3};  // CxR
    float a_values_0[] = {
        -2,  2, -1,
         6, -6,  7,
         3, -8,  4
    };
    tensor_t* A = new_tensor(a_shape_0, 2, a_values_0);

    failure_count += test_lu_decomposition_single(A);
    tensor_free(A);
    
    printf("--- Test 1 b): LU Decomposition ---\n");

    int a_shape_1[] = {4, 5};  // CxR
    float a_values_1[] = {
        0, 1, 2, 1, 2,
        1, 0, 0, 0, 1,
        2, 1, 2, 1, 5,
        1, 2, 4, 3, 6
    };
    A = new_tensor(a_shape_1, 2, a_values_1);

    failure_count += test_lu_decomposition_single(A);
    tensor_free(A);

    printf("--- Test 1 c): LU Decomposition ---\n");

    int a_shape_2[] = {3, 3};  // CxR
    float a_values_2[] = {
        -1.5,  2.1,  6.4,
           2,  3.9,  3.1,
           5,    3,    2,
    };
    A = new_tensor(a_shape_2, 2, a_values_2);

    failure_count += test_lu_decomposition_single(A);
    tensor_free(A);

    return failure_count;
}

int test_determinant()
{
    int failure_count = 0;

    printf("--- Test 2 a): Determinant ---\n");

    int a_shape_0[] = {2, 2};  // CxR
    float a_values_0[] = {
        8, 5,
        4, 5
    };
    tensor_t* A = new_tensor(a_shape_0, 2, a_values_0);

    float det = tensor_determinant(A);

    printf("----- A -----\n");
    print_tensor(A);
    printf("Determinant: %f\n", det);

    failure_count += det == 20.f ? 0 : 1;

    printf("--- Test 2 b): Determinant ---\n");

    int a_shape_1[] = {3, 3};  // CxR
    float a_values_1[] = {
         1, -2,  3,
         2,  0,  3,
         1,  5,  4
    };
    A = new_tensor(a_shape_1, 2, a_values_1);

    det = tensor_determinant(A);

    printf("----- A -----\n");
    print_tensor(A);
    printf("Determinant: %f\n", det);

    failure_count += det == 25.f ? 0 : 1;

    int a_shape_2[] = {4, 4};  // CxR
    float a_values_2[] = {
         1,  3,  1,  4,
         3,  9,  5, 15,
         0,  2,  1,  1,
         0,  4,  2,  3
    };
    A = new_tensor(a_shape_2, 2, a_values_2);

    det = tensor_determinant(A);

    printf("----- A -----\n");
    print_tensor(A);
    printf("Determinant: %f\n", det);

    failure_count += det == -4.f ? 0 : 1;

    tensor_free(A);

    return failure_count;
}

int test_inverse_single(tensor_t* A)
{
    tensor_t* I = tensor_copy_shape(A);
    tensor_identity(I);

    tensor_t* A_inv = tensor_inverse_give(A);

    printf("----- A -----\n");
    print_tensor(A);

    printf("----- A^-1 -----\n");
    print_tensor(A_inv);

    
    tensor_t* A_A_inv = tensor_mult_give(A, A_inv);

    printf("----- A @ inv_A -----\n");
    print_tensor(A_A_inv);

    int failure = tensors_equal(I, A_A_inv) ? 0 : 1;

    tensor_free(A_inv);
    tensor_free(I);
    tensor_free(A_A_inv);

    return failure;
}

int test_inverse()
{
    int failure_count = 0;

    printf("--- Test 3 a): Inverse ---\n");

    int a_shape[] = {2, 2};  // CxR
    float a_values[] = {
         2, 1,
         1, 3
    };
    tensor_t* A = new_tensor(a_shape, 2, a_values);

    failure_count += test_inverse_single(A);

    tensor_free(A);

    printf("--- Test 3 b): Inverse ---\n");

    int a_shape_0[] = {3, 3};  // CxR
    float a_values_0[] = {
         2,  1,  1,
         4, -6,  0,
        -2,  7,  2
    };
    A = new_tensor(a_shape_0, 2, a_values_0);

    failure_count += test_inverse_single(A);

    tensor_free(A);

    printf("--- Test 3 c): Inverse ---\n");

    int a_shape_1[] = {3, 3};  // CxR
    float a_values_1[] = {
          3,  -5,   9,
         10,   8,  -6,
          7,  15,  -2
    };
    A = new_tensor(a_shape_1, 2, a_values_1);

    failure_count += test_inverse_single(A);

    tensor_free(A);

    printf("--- Test 3 d): Inverse ---\n");

    int a_shape_2[] = {4, 4};  // CxR
    float a_values_2[] = {
        1, 1, 1, 0,
        0, 3, 1, 2,
        2, 3, 1, 0,
        1, 0, 2, 1
    };
    A = new_tensor(a_shape_2, 2, a_values_2);

    failure_count += test_inverse_single(A);

    tensor_free(A);

    printf("--- Test 3 e): Inverse ---\n");

    int a_shape_3[] = {5, 5};  // CxR
    float a_values_3[] = {
         1,  1,  1,  0,  1,
         0, -3,  1, -2,  4,
         2,  3,  1,  0,  6,
         1, -2, -2,  1, -1,
         1,  0,  2, -1, -1,
         1,  3, -4, -4,  0
    };
    A = new_tensor(a_shape_3, 2, a_values_3);

    failure_count += test_inverse_single(A);

    tensor_free(A);

    return failure_count;
}

int main()
{
    int failures_count = 0;

    failures_count += test_lu_decomposition();
    failures_count += test_determinant();
    failures_count += test_inverse();

    if (failures_count > 0) {
        printf("\033[1;31mFailed %d test(s)!\033[0m\n", failures_count);
    } else {
        printf("\033[1;32mAll tests passed! :)\033[0m\n");
    }
    return 0;
}
